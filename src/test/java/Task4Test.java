...
public class Task4Test {
    ...
    public static class MergeListTestCase {
        ...
        public MergeListTestCase(int[] list1, int[] list2, int[] expect) {
            this.list1 = Utils.arrayToListWithoutDummy(list1);
            this.list2 = Utils.arrayToListWithoutDummy(list2);
            this.expect = expect;
        }
    }
    ...
    public void testMergeList() {
        ...
        for (MergeListTestCase testCase : testCases) {
            assertArrayEquals(testCase.expect,
                    Utils.listWithoutDummyToArray(SortList.mergeSortedSingleLinkedLists(
                            testCase.list1,
                            testCase.list2)));
        }
    }
    ...
    public void testSortList() {
        ...
        for (SortListTestCase testCase : testCases) {
            int[] actual = Utils.listWithoutDummyToArray(SortList.sortList(testCase.head));
    ...
    public void testFindMidAndBreak() {
        ...
        for (int i = 0; i < inputs.length; i++) {
            ...
            ListNode list = Utils.arrayToListWithoutDummy(input);
        ...
        assertNull(SortList.findMidAndBreak(Utils.arrayToListWithoutDummy(emptyInput)));
    }

    private List<SortListTestCase> getSortListTestCases(int[][] inputs, int[][] expects) {
        List<SortListTestCase> testCases = new ArrayList<>();
        for (int i = 0; i < inputs.length; i++) {
            SortListTestCase testCase = new SortListTestCase(Utils.arrayToListWithoutDummy(inputs[i]), expects[i]);
